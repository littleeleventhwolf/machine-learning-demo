...
def train(mnist):
    # 定义输入输出placeholder。
    x = tf.placeholder(tf.float32, [BATCH_SIZE, mnist_inference_conv.IMAGE_SIZE,
                    mnist_inference_conv.IMAGE_SIZE,
                    mnist_inference_conv.NUM_CHANNELS], name='x-input')
    y_ = tf.placeholder(tf.float32, [BATCH_SIZE, mnist_inference_conv.NUM_LABELS], name='y-input')
    ...
    # 和2.1小节样例中类似地定义损失函数、学习率、滑动平均操作以及训练过程。
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            reshaped_xs = np.reshape(xs, (BATCH_SIZE, mnist_inference_conv.IMAGE_SIZE, 
