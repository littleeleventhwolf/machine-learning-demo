...
# 这个张量名称就是'pool_3/_reshape:0'。在训练模型时，可以通过tensor.name来获取张量的名称。
...
# 因为一个训练数据会被使用多次，所以可以将原始图像通过Inception-v3模型计算得到的特征向量
# 保存在文件中，免去重复的计算。下面的变量定义了这些文件的存放地址。
...
def create_image_lists(testing_percentage, validation_percentage):
    # 得到的所有图片都存在result这个字典（dictionary）里。这个字典的key为类别的名称，
...
# 这个函数获取一张图片经过Inception-v3模型处理之后的特征向量。这个函数会先试图寻找已经计算
...
def main(_):
    ...
    # 里面保存了每一个节点取值的计算方法以及变量的取值。TensorFlow模型的持久化问题在上一篇中
    # 有详细的介绍。
    ...
    # 定义新的神经网络输入，这个输入就是新的图片经过Inception-v3模型前向传播到达瓶颈层时
    ...
    # 图片抽象为了更加容易分类的特征向量了，所以不需要再训练那么复杂的神经网络来完成这个
    # 新的分类任务。
    ...
    with tf.Session() as sess:
        ...
        print('Final test_accuracy = %.1f%%' % (test_accuracy * 100))
