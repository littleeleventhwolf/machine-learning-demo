...
def inference(input_tensor, train, regularizer):
    ...
    # 和标准LeNet-5模型不大一样，这里定义的卷积层输入为28×28×1的原始MNIST图片像素。因为卷积层中使用了全0填充，
    # 所以输出为28×28×32的矩阵。
    ...
    # 所以在这里需要将这个7×7×64的矩阵拉直成一个向量。pool2.get_shapes函数可以得到第四层输出矩阵的维度而不需要手工计算。
    ...
    # 声明第五层全连接层的变量并实现前向传播过程。这一层的输入是拉直之后的一组向量，向量长度为3136，输出的一组长度为512的向量。
